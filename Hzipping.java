...
public class Hzipping
{
	...
	static class TREE implements Comparable<TREE> 
	{
		...
		{
			if(this.Freqnc<T.Freqnc)return -1;
			if(this.Freqnc>T.Freqnc)return 1;
			return 0;
		}
	}
 ...
 * *********************************************************************************/
	public static void fredfs(TREE now)
	{
			...
			if(now.Lchild!=null) fredfs(now.Lchild);
			if(now.Rchild!=null) fredfs(now.Rchild); 
	}
/**********************************************************************************/
 ...
 *********************************************************************************/
	public static void dfs(TREE now,String st)
	{
		...
		if(now.Lchild!=null) dfs(now.Lchild,st+"0");
		if(now.Rchild!=null) dfs(now.Rchild,st+"1");
	}
/**********************************************************************************/
...
/*******************************************************************************
 *Making all the nodes in a priority Q
 *making the tree
 *******************************************************************************/
...
/*******************************************************************************/
	...
	{
		initHzipping();
		CalFreq("in.txt");       						// calculate the frequency of each digit
		MakeNode();                						// makeing corresponding nodes
		if(cnt>1) dfs(Root,"");   		     		   	// dfs to make the codes
		fakezip("in.txt");             				    // fake zip file which will have the binary of the input to fakezipped.txt  file
		realzip("fakezipped.txt","in.txt"+".huffz");    // making the real zip according the fakezip.txt file
		initHzipping();
	...
	public static void beginHzipping(String arg1) 
	{
		...
		realzip("fakezipped.txt",arg1+".huffz");    // making the real zip according the fakezip.txt file
		initHzipping();
	} 
} 